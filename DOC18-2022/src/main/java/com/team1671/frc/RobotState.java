    ...
    public void addVisionUpdate(double timestamp, List<TargetInfo> vision_update) {
        ...
        public synchronized Optional<ShooterAimingParameters> getAimingParameters() {
			...
			if (!reports.isEmpty()) {
                ...
                Translation2d bucketPosition = lastKnownTargetPosition = report.field_to_goal;

                Translation2d turret_to_bucket = vehicle_pose.transformBy(kVehicleToTurretFixed)
                    .getTranslation().inverse().translateBy(bucketPosition);

                Pose2d turret_to_goal_robot_centric = vehicle_pose.transformBy(kVehicleToTurretFixed)
                    .inverse().transformBy(Pose2d.fromTranslation(bucketPosition));
                
                isTrackerEmpty = false;


				ShooterAimingParameters params = new ShooterAimingParameters(turret_to_bucket.norm(), 
						turret_to_goal_robot_centric.getTranslation().direction(), turret_to_bucket, report.latest_timestamp, report.stability);
