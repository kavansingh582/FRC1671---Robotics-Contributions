...
public class LimelightProcessor implements Loop{
	...
	public void onLoop(double timestamp){
		...
		//System.out.println(Math.tan(Math.toRadians(target.get(0).getDouble(0))));
		//SmartDashboard.putNumber("limelight number", NetworkTableInstance.getDefault().getTable("limelight").getEntry("tx").getDouble(0.));
	...
	public double getX(){
		return target.get(0).getDouble(0);
	}

	public double getY(){
		return target.get(1).getDouble(0);
	}

	public double getDistFromTarget(){
		return (Constants.kVisionTargetHeight - Constants.kLimelightHeight) / Math.tan(Math.toRadians(Constants.kLimelightMountingAngle + getY()));
	}

	public boolean targetExists(){
		return target.get(3).getDouble(0) != 0;
	}
